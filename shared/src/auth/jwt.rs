...
pub fn generate_refresh_token(config: &AuthConfig, user_id: &Uuid) -> Result<String, AuthError> {
    let now = current_timestamp()?;
...
pub fn validate_access_token(
    ...
    let decoding_key = DecodingKey::from_secret(&config.access_token_secret);
...
pub fn validate_refresh_token(
    ...
    let decoding_key = DecodingKey::from_secret(&config.refresh_token_secret);
    let mut validation = Validation::new(Algorithm::HS256);
    validation.validate_exp = true;
    validation.leeway = 0;
...
pub fn refresh_access(
    ...
    let claims = validate_refresh_token(config, refresh_token)?;
    let new_access_token = generate_access_token(config, &claims.sub, name, role)?;
    Ok(new_access_token)
}
...
mod tests {
    ...
    fn test_generate_and_validate_access_token() {
        ...
        let token = generate_access_token(&config, &user_id, name, role)
            .expect("Failed to generate access token");

        let claims =
            validate_access_token(&config, &token).expect("Failed to validate access token");
    ...
    fn test_generate_and_validate_refresh_token() {
        ...
        let token =
            generate_refresh_token(&config, &user_id).expect("Failed to generate refresh token");

        let claims =
            validate_refresh_token(&config, &token).expect("Failed to validate refresh token");
    ...
    fn test_access_token_invalid_signature() {
        ...
        let token = generate_access_token(&config, &user_id, "Name", Role::Buyer).unwrap();

        let mut wrong_config = AuthConfig::for_tests();
        wrong_config.access_token_secret = b"different_secret".to_vec();
    ...
    fn test_refresh_token_invalid_signature() {
        ...
        let mut wrong_config = AuthConfig::for_tests();
        wrong_config.refresh_token_secret = b"different_secret".to_vec();

        let result = validate_refresh_token(&wrong_config, &token);
    ...
    fn test_access_token_expiration() {
        ...
        assert!(validate_access_token(&config, &token).is_ok());
    ...
    fn test_refresh_token_expiration() {
        ...
        assert!(validate_refresh_token(&config, &token).is_ok());

        thread::sleep(Duration::from_secs(2));

        assert!(
            validate_refresh_token(&config, &token).is_err(),
    ...
    fn test_refresh_access_token_flow() {
        ...
        let new_access_token = refresh_access(&config, &refresh_token, new_name, new_role)
            .expect("Failed to refresh access token");

        let claims = validate_access_token(&config, &new_access_token)
    ...
    fn test_refresh_with_invalid_token() {
        ...
        let result = refresh_access(&config, "invalid.token.here", "Name", Role::Buyer);
        assert!(result.is_err());

        let access_token = generate_access_token(&config, &user_id, "Name", Role::Buyer).unwrap();
        let result = refresh_access(&config, &access_token, "Name", Role::Buyer);
        assert!(result.is_err());
    }
}
